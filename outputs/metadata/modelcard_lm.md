...
This model uses a Linear Mixed Effects architecture to predict regional GDP (by NUTS-3 region) based on wastewater metabolite concentrations of illicit drugs and longitudinal time data, utilizing the specific city region as a random effect to account for baseline economic differences.

## Model Details

### Model Description

This is a Linear Mixed Effects Model developed in R using the `lme4` package (version 2.0.1). 
It was trained to explore a spurious correlation between the economic productivity of European cities (GDP) and their illicit drug consumption. 
The model uses fixed effects for year and various drug metabolites (MDMA, amphetamine, cocaine, methamphetamine) and a random intercept for the specific NUTS-3 region (`nuts_code`) to capture between-region variation.
...
-   **Model type:** Nested Linear Mixed Model (LMM) / Regression Architecture
-   **Language(s) (NLP):** R
...
Instead of learning a genuine relationship between illicit drugs and GDP, the model is likely just memorizing the baseline GDP of specific regions through its random intercept. Future iterations of this experiment could remove the city variable entirely to evaluate the true predictive power of the wastewater metabolites alone.
Additional data limitations include a short time series and a low number of overall observations. There is also an imperfect estimation of populations in the catchment areas of wastewater treatment plants, especially during major events. Statistically, the model assumes normally distributed random effects and residuals, is limited to random intercepts only
(no random slopes), and is sensitive to the scaling of continuous predictors.
...
When reusing this model contextual framing is important. Any repository or generated reports should prominently feature a disclaimer regarding the spurious nature of the correlation being explored.
...
```r
# load singular model 
saved_model <- readRDS("../outputs/models/lm_model.rds")
...
-   **Training regime:** Standard statistical fitting via Restricted Maximum Likelihood (REML = true) using the lme4 package in R.
-   **Formula:** gdp ~ ref_year + mdma + amphetamine + cocaine + methamphetamine + (1 | nuts_code)
-   **Optimizer:** bobyqa
...
The evaluation utilized a holdout test dataset of 97 observations (20%) generated during the preprocessing phase via random sampling.
...
The Nested Linear Mixed Model (LMM) performed the best out of the three architectures evaluated, marginally outperforming the simple linear model.
...
The evaluation reveals that the Nested Linear Mixed Model (LMM) performs best. 
This is likely because, compared to the simple Linear Model, it represents the hierarchical reality of the regional data better—though the overall difference is marginal.
...
Nested Linear Mixed Model with fixed intercepts for time and metabolic concentrations, and random intercepts for municipal geography.
...
- **Key Dependencies:** lme4 (v2.0.1), `tidymodels_1.5.0`, `tidyverse_2.0.0`
